...
export default function CourseConceptsPage() {
  ...
  async function load() {
    const [courseRes, conceptsRes] = await Promise.all([
      ...
      fetch(`/api/concepts?courseId=${courseId}`),
    ]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 flex-wrap">
        <Link href="/courses" className="text-sm hover:underline" style={{ color: "#0cc0df" }}>← Courses</Link>
        <span style={{ color: "var(--border)" }}>/</span>
        <Link href={`/courses/${courseId}`} className="text-sm hover:underline" style={{ color: "#0cc0df" }}>{courseName}</Link>
        <span style={{ color: "var(--border)" }}>/</span>
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>
            Topic categories for <span style={{ color: "var(--text-secondary)" }}>{courseName}</span>. Students progress through these in order.
          </p>
        </div>
        <button
