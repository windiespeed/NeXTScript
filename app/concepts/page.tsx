...
export default function ConceptsPage() {
  ...
  async function handleSeed() {
    setSeeding(true);
    setError("");
    const res = await fetch("/api/concepts/seed", { method: "POST" });
    if (res.ok) {
      await load();
    } else {
      const data = await res.json();
  ...
  async function handleAdd() {
    ...
    const res = await fetch("/api/concepts", {
      ...
      body: JSON.stringify({ label: newLabel.trim(), slug: newSlug.trim(), description: newDesc.trim(), order: maxOrder + 1 }),
    });
    if (res.ok) {
  ...
  async function handleDelete(id: string) {
    if (!confirm("Delete this concept? Exercises using it will keep their concept slug but it won't appear in the list.")) return;
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>
            Define the topic categories for your exercises. Students progress through concepts in order.
          </p>
        </div>
        <button
        ...
        <div className={cardClass} style={cardStyle}>
          <div className="text-center py-8 space-y-4">
            ...
            <p className="text-xs" style={{ color: "var(--text-muted)" }}>
              Create your own concepts for any subject, or seed the JavaScript defaults to get started quickly.
            </p>
            <button
              onClick={handleSeed}
              disabled={seeding}
                ...
                <div className="rounded-3xl px-4 py-3 flex items-start gap-3" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
                  ...
                  <div className="flex flex-col gap-0.5 pt-0.5 shrink-0">
                    <button onClick={() => moveUp(idx)} disabled={idx === 0}
                      ...
                      style={{ color: "var(--text-muted)" }} aria-label="Move up">
                    ...
                    <button onClick={() => moveDown(idx)} disabled={idx === concepts.length - 1}
                      className="p-0.5 rounded transition hover:opacity-80 disabled:opacity-20"
                      style={{ color: "var(--text-muted)" }} aria-label="Move down">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button onClick={() => { setEditId(concept.id); setEditLabel(concept.label); setEditDesc(concept.description); setError(""); }}
                      className="text-xs transition hover:opacity-80" style={{ color: "#0cc0df" }}>
                      Edit
                    </button>
                    <button onClick={() => handleDelete(concept.id)}
                      className="text-xs transition hover:opacity-80" style={{ color: "#ef4444" }}>
                      Delete
                    </button>
                  </div>
                </div>
      ...
      {concepts.length > 0 && (
        <div className="pb-8">
          <p className="text-xs" style={{ color: "var(--text-muted)" }}>
            Concept slugs are permanent — they link to existing exercises. You can edit labels and descriptions at any time.
          </p>
        </div>
      )}
    </div>
