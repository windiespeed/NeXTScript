    ...
    <div className="space-y-6">
      <div className="rounded-3xl bg-gradient-to-br from-[#08090f] from-30% to-[#0cc0df]/30 px-6 py-6 shadow" style={{ border: "1px solid rgba(12,192,223,0.2)" }}>
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          {course.subject && <span className="rounded-full bg-white/10 px-2.5 py-0.5 text-xs text-white/80">{course.subject}</span>}
          {course.gradeLevel && <span className="rounded-full bg-white/10 px-2.5 py-0.5 text-xs text-white/80">{course.gradeLevel}</span>}
          {course.term && <span className="rounded-full bg-white/10 px-2.5 py-0.5 text-xs text-white/80">{course.term}</span>}
      ...
      {course.lessons.length === 0 ? (
        ...
        <div className="space-y-3">
          <p className="text-xs font-semibold uppercase tracking-widest" style={{ color: "var(--sidebar-label)" }}>Course Lessons</p>
          <div className="rounded-full overflow-hidden" style={{ border: "1px solid var(--border)" }}>
            {course.lessons.map((lesson, i) => (
              <div key={lesson.id} className="flex items-start gap-4 px-5 py-4"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {lesson.subtitle && <p className="text-xs mt-0.5" style={{ color: "var(--text-secondary)" }}>{lesson.subtitle}</p>}
                  {lesson.topics && <p className="text-xs mt-2 line-clamp-2 text-[#0cc0df]">{lesson.topics}</p>}
                  <div className="flex flex-wrap items-center gap-3 mt-2">
                    ...
                    {lesson.tag && (
                      <span className="rounded px-1.5 py-0.5 text-[10px] font-semibold text-[#0cc0df]" style={{ background: "var(--accent-bg)" }}>{lesson.tag}</span>
                    )}
                  </div>
                </div>
                {lesson.folderUrl && (
