...
export default function QuizzesPage() {
  ...
  const [existingQuizzes, setExistingQuizzes] = useState<SavedProject[]>([]);
  ...
  useEffect(() => {
    Promise.all([
      ...
      setExistingQuizzes(Array.isArray(q) ? q : []);
  ...
  const scopeLessons: Lesson[] = (() => {
    if (scope === "course") return courseLessons;
    if (scope === "module" && selectedModule) {
      return selectedModule.lessonIds.map(lid => courseLessons.find(l => l.id === lid)).filter(Boolean) as Lesson[];
    }
    if (scope === "lesson") return courseLessons;
    return [];
  })();
  ...
  function handleCourseChange(courseId: string) {
    ...
    if (course) {
      if (scope === "course") setQuizTitle(`${course.title} — Final Quiz`);
    }
  }
  ...
  async function handleGenerate() {
    if (!quizTitle.trim()) { setGenerateError("Quiz title is required."); return; }
    if (questions.length === 0) { setGenerateError("Add at least one question."); return; }
    setGenerating(true);
    setGenerateError("");
    setDoneUrl("");
    try {
      ...
      setDoneUrl(data.url);
      setExistingQuizzes(prev => [{ id: data.id, type: "form", isQuiz: true, title: quizTitle, url: data.url, lessonIds: resolvedLessonIds, moduleId: scope === "module" ? selectedModuleId : undefined, courseId: selectedCourseId || undefined, userId: "", createdAt: new Date().toISOString() }, ...prev]);
    } catch (err: any) {
  ...
  const canGenerate = quizTitle.trim() && questions.length > 0;

  return (
    <div className="max-w-3xl space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>Quiz Builder</h1>
      ...
      {/* ── Lesson / Module / Course Selection ── */}
      {scope !== "standalone" && (
        <div className={cardClass} style={cardStyle}>
          ...
          {scope === "module" && selectedCourseId && (
            <div>
              ...
              {modules.length === 0 ? (
                <p className="text-xs" style={{ color: "var(--text-muted)" }}>This course has no modules yet. Add modules from the course page.</p>
              ) : (
                <select
          ...
          {/* Course scope — show lesson count */}
        ...
        <div className={cardClass} style={cardStyle}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="number" min={0} max={50} value={mcCount}
                onChange={e => setMcCount(Math.min(50, Math.max(0, Number(e.target.value))))}
            ...
            <div>
              ...
              <input
                type="number" min={0} max={50} value={saCount}
                onChange={e => setSaCount(Math.min(50, Math.max(0, Number(e.target.value))))}
                className={inputClass} style={inputStyle}
              />
            </div>
          </div>
                          ...
                          <div key={oi} className="flex items-center gap-2">
                            ...
                            {q.options.length > 2 && (
                              <button
                                ...
                                className="text-xs hover:text-red-500 transition shrink-0"
      ...
      <div className="flex items-center gap-4">
        ...
        >
          {generating ? "Creating Quiz in Drive…" : "Create Quiz in Google Drive"}
        </button>
        {generateError && <p className="text-xs text-red-500">{generateError}</p>}
        {doneUrl && (
          <a href={doneUrl} target="_blank" rel="noopener noreferrer" className="text-xs font-semibold text-[#2dd4a0] hover:underline">
            Open Quiz ↗
          </a>
        )}
      </div>
      ...
      {existingQuizzes.length > 0 && (
        <div className={cardClass} style={cardStyle}>
          <p className={sectionLabel}>Your Quizzes</p>
          <div className="space-y-2">
            {existingQuizzes.map(q => (
              <div key={q.id} className="flex items-center justify-between gap-3 px-3 py-2.5 rounded-2xl" style={{ background: "var(--bg-card-hover)" }}>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold truncate" style={{ color: "var(--text-primary)" }}>{q.title}</p>
                  {q.lessonIds && q.lessonIds.length > 0 && (
                    <p className="text-[10px]" style={{ color: "var(--text-muted)" }}>
                      Linked to {q.lessonIds.length} lesson{q.lessonIds.length !== 1 ? "s" : ""}
                    </p>
                  )}
                </div>
                <a
                  ...
                  className="rounded-full px-3 py-1 text-[10px] font-semibold transition hover:opacity-80 shrink-0"
