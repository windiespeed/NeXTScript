    ...
    <div className="max-w-3xl space-y-5">
      ...
      <div className={cardClass} style={cardStyle}>
        ...
        <div className="flex items-center gap-3">
          ...
          {lesson.folderUrl && (
            <a href={lesson.folderUrl} target="_blank" rel="noopener noreferrer" className="text-xs font-semibold hover:underline" style={{ color: "#2dd4a0" }}>
              Open Drive Folder ↗
            </a>
          )}
        </div>
      ...
      <div className={cardClass} style={cardStyle}>
        ...
        <div className="space-y-2">
          ...
          {lesson.overviewUrl ? (
            <DocRow
              icon={RESOURCE_ICON.doc}
              color="#0cc0df"
              label="Overview Doc"
              badge="Generated"
              badgeColor="#2dd4a0"
              url={lesson.overviewUrl}
            />
          ) : (
            <DocRow
              icon={RESOURCE_ICON.doc}
          ...
          {quizDrafts.map(q => (
            <DocRow
              key={q.id}
              icon={RESOURCE_ICON.form}
              color="#ff8c4a"
              label={q.title}
              badge="Quiz Draft"
              badgeColor="#ff8c4a"
              editHref={`/quizzes/${q.id}`}
              editLabel="Edit Quiz"
            />
          ))}
          ...
          {!deck && quizProjects.length === 0 && !lesson.overviewUrl && (
            <p className="text-xs text-center py-4" style={{ color: "var(--text-muted)" }}>
              No documents generated yet — use the Generate panel above.
            </p>
          )}
        </div>
      </div>
      ...
      <div className={cardClass} style={cardStyle}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => { setAddingBlankDoc(true); setAddingLink(false); }}
            ...
            <button
              onClick={() => { setAddingLink(true); setAddingBlankDoc(false); }}
              className="rounded-full px-3 py-1 text-xs font-semibold transition"
              style={{ background: "var(--bg-card-hover)", border: "1px solid var(--border)", color: "var(--text-secondary)" }}
            >
              + Add Link
